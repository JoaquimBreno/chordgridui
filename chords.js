...
var ChordJS = (function(){
    ...
    var ChordBoxImage = function(name, chord, fingers, size, stringNames) {
        ...
        var _foregroundBrush = '#FFFFFF';
        var _backgroundBrush = 'transparent';
        var _fontBrush = '#000000';
        ...
        var InitializeSizes = function() {
            ...
            _xstart = _fretWidth;
            _ystart = Math.round( _nutHeight + 1.7 * _markerWidth);
        ...
        var ParseChord = function(chord) {
            console.log("READING CHORD:")
            console.log(chord)
            if (chord == null || typeof chord == 'undefined' || !chord.match(/[\dxXa-f]{6}|((1|2)?[\dxX]-){5}(1|2)?[\dxX]/)) {
                _error = true;
                console.log("erro")
            } else {
                var parts;
                ...
                for (var i = 0; i < 6; i++) {
                    ...
                    if (fret.toUpperCase() == "X") {
                        ...
                        if (isNaN(parseInt(fret))) {
                            // Assume letter notation
                            var letterValue = fret.charCodeAt(0) - 87; // 'a' is 97 in ASCII
                            _chordPositions[i] = letterValue;
                        } else {
                            _chordPositions[i] = parseInt(fret);
                        }
            ...
            for (var i = 0; i < _chordPositions.length; i++) {
                ...
                if (relativePos > 0) {
                    ...
                    if (finger != NO_FINGER) {
                        ...
                        _graphics.DrawString(finger.toString(), font, _fontBrush, xpos - (0.5 * charSize.Width) + _dotWidth/2, ypos - (0.5 * charSize.Height) + _dotWidth/2);
                    }
                } else if (absolutePos == OPEN) {
                    var pen = Pen(_foregroundBrush, _lineWidth);
                    var ypos = _ystart - _fretWidth;
        ...
        var DrawChordName = function() {
            ...
            // chordName.textContent = _chordName;
